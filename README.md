...
```
 文档
  │  切分（结构化：过滤目录/页码/OCR 噪声 → 按标题分节 → ~900 字一段 → 标注类型）
  ▼
 片段 ──► 领域 schema 归纳（概念表 + 别名）
  │
  │  生成 ≥100 条分层测试集 → 人工校验后冻结
  ▼
 抽取（两阶段：先抽节点 → 实体消歧 → 在已知节点之间抽关系） ──► Neo4j
  ▼
 评测（节点/关系覆盖率，确定性计算） + 诊断（漏节点 / 漏关系 / 错连 / 粒度，按类型统计）
  ▼
 修正（据诊断给下一轮抽取注入针对性指令 + 类型级 few-shot） ──┐
  ▲                                                          │
  └──────────────── 未收敛则再抽一轮 ◄────────────────────────┘
  ▼ 收敛（节点≥0.85，关系≥0.75，回答≥0.80 且不再提升）
 知识图谱 + 问答（检索子图 → 作答）
```
